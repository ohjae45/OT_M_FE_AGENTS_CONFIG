...
│   └── skills/         # agent-docs/skills/* symlink
└── scripts/
...
│   └── skills/             ← COMMON_AGENT_CONFIG/agent-docs/skills/*에서 생성 (managed)
└── .agents/
    └── skills/
        └── <skill>/SKILL.md ← COMMON_AGENT_CONFIG/agent-docs/skills/*에서 생성 (managed)
```
...
COMMON_AGENT_CONFIG의 `.claude/skills/*.md`는 복사본이 아니라 `agent-docs/skills/*.md`를 가리키는 symlink입니다. 원본 레포에서 Claude가 기존 경로로 스킬을 읽을 수 있게 하기 위한 진입점입니다.
...
- `COMMON_AGENT_CONFIG/agent-docs/skills/*` → `.claude/skills/*`
- `COMMON_AGENT_CONFIG/agent-docs/skills/*` → `.agents/skills/<skill>/SKILL.md`
- `scripts/sync-agent-config.sh` → `scripts/sync-agent-config.sh`

이전 동기화로 target repo에 `agent-docs/skills/*.md`가 생겨 있으면, 원본과 내용이 같은 파일만 다음 sync에서 삭제합니다.
