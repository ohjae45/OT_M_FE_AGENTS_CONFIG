...
**병렬 + SendMessage 협업은 기본 모드가 아니다.** 서브 에이전트 간 SendMessage가 동작하려면 (1) 양쪽 에이전트 정의에 SendMessage tool 권한이 명시되어 있어야 하고, (2) 상대방 에이전트 ID/이름을 알 수 있어야 한다. 이 전제가 실제 환경에서 검증되기 전까지는 순차 실행 + 파일 핸드오프를 사용한다. 병렬화는 [#병렬화-옵션-검증-후](#병렬화-옵션-검증-후) 참고.
