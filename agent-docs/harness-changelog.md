...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-07-20 | 공통 가드레일("주의·금지") + 조건부 테스트 게이트 도입 — 각 에이전트에 단정 금지·스코프 경계·의도 혼합 금지·기존 분기 보존·조용한 축소 금지 원칙을 주입하고, skai-qa가 순수 로직·버그 수정·경계 조건에 한해 vitest 테스트를 선택적으로 남기도록 함(designer 조건부 호출과 동형) | `agent-docs/skills/skai-orchestrator.md`, `agent-docs/agents/skai-{analyst,builder,integration,qa}.md`, `agent-docs/harness-changelog.md` | "해야 할 것"만 있고 "하지 말 것/주의할 것"이 없어 단정·스코프 이탈 실수가 반복됨. 테스트는 매번 돌리면 파이프라인이 느려지므로 필요 시에만 발동하는 선택적 흐름으로 보완 |
| 2026-05-22 | Phase 0.5 (skai-designer) 추가 — 디자인 시안·스크린샷·UI 이미지 또는 자연어 UI 설명을 시각 명세(`_workspace/00_designer_spec.md`)로 변환해 analyst·builder가 시각 토큰·상태·반응형을 일관되게 따르도록 함 | `agent-docs/agents/skai-designer.md`, `agent-docs/skills/skai-orchestrator.md`, `agent-docs/agents/skai-{analyst,builder,qa}.md` | "구조만 받고 끝내면 구현이 시각과 어긋난다"는 피드백을 단일 파이프라인 단계로 캡슐화. SKAI 브랜드 컬러(`--ws-accent-*`) 우선 매핑 원칙 표준화 |
