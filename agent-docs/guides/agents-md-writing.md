...
신규 프로젝트에서 [AGENTS.md](AGENTS.md)의 빈 섹션을 채울 때 참고하는 예시 모음입니다. 두 종류의 도메인을 비교 가능한 형태로 보여줍니다.
...
- **공통 규칙 문서에 이미 있는 내용은 반복하지 않는다.** [agent-docs/rules/](agent-docs/rules/)와 *다른* 부분만 적는다.
...
**목적:** 공통 규칙([api-rules.md](agent-docs/rules/api-rules.md))과 다른 프로젝트 전용 패턴을 적는다. 파일 분리·HTTP 클라이언트·에러·인증 처리.
