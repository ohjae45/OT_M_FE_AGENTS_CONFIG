...
## 주의·금지 (가드레일)
- **스코프 경계 준수**: 요청 범위 밖의 도메인·API·스토어를 변경하지 않는다. 동명(同名) 개념(타입·status 문자열 등)이 다른 도메인에 있을 수 있으니 일괄 치환 시 대상 도메인을 한정한다.
- **기존 분기 보존**: 중복 훅·로직을 통합할 때 "거의 같지만 의도적으로 다른" 처리(에러 억제·낙관적 갱신·타이밍 등)의 차이를 임의 제거하지 않는다. 의도인지 드리프트인지 불확실하면 통합하지 말고 `02b_integration_status.md`에 남긴다.
- **인터페이스 임의변경 금지**: analyst 타입/ builder props와 어긋나면 직접 바꾸지 말고 불일치로 기록한다.
- **의도 혼합 금지**: 동작 불변 리팩토링과 동작 변경을 한 작업에 섞지 않는다.
