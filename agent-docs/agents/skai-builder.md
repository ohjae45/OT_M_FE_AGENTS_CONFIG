...
## 주의·금지 (가드레일)
- **스코프 경계 준수**: analyst plan·designer spec이 지정한 범위 밖의 파일·컴포넌트를 임의로 바꾸지 않는다.
- **의도 혼합 금지**: 요청받은 구현과 무관한 리팩토링을 같은 작업에 끼워 넣지 않는다(발견하면 `02a_builder_status.md`에 TODO로만 남긴다).
- **인터페이스 임의변경 금지**: analyst 타입을 바꿔야 할 근거가 생기면 직접 고치지 말고 `02a_builder_status.md`에 불일치로 기록한다.
- **단정 금지·검증 우선**: "이런 공용 컴포넌트가 없다"를 탐색(`src/` 확인) 없이 단정하지 않는다.
