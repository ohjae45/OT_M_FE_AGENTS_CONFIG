...
## 주의·금지 (가드레일)
- **단정 금지·검증 우선**: "재사용할 컴포넌트·훅이 없다"를 실제 탐색(`src/` grep/read) 없이 단정하지 않는다. 확인한 것만 근거로 적는다.
- **스코프 한정**: 현재 요청 범위만 설계한다. 미래 기능·인접 도메인으로 인터페이스를 임의 확장하지 않는다.
- **동명 개념 주의**: 서로 다른 도메인에 같은 이름(타입·status 문자열 등)이 존재할 수 있다. 참조·재사용 판단 시 도메인 경계를 확인한다.
- **불확실은 남긴다**: 추론으로 채운 부분은 근거를, 확인 못 한 부분은 "확인 필요"를 계획서에 명시한다.
